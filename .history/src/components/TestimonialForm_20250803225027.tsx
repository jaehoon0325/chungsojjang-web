...
const TestimonialForm: React.FC<TestimonialFormProps> = ({ onSubmitSuccess, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 후기 데이터 저장 (undefined 값 제거)
      const testimonialData: Partial<CustomerTestimonial> & {
        customerName: string;
        serviceType: string;
        review: string;
        rating: number;
      ...
      if (user.displayName) {
        testimonialData.authorName = user.displayName;
      }
      
      if (user.email) {
        testimonialData.authorEmail = user.email;
      }
      
      if (user.photoURL) {
        testimonialData.authorPhotoURL = user.photoURL;
      }
      ...
      if (error instanceof Error) {
        if (error.message.includes('permission-denied')) {
          errorMessage = 'Firestore 권한이 없습니다. 관리자에게 문의하세요.';
